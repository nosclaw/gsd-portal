...
所有 WS 在**开始工作、进度更新、完成**时都需要向 Tech Manager 汇报。跨机器 WS 之间通过 Portal WebSocket Hub 实时通信。

详细设计见 **[Portal Messenger 设计文档](./portal-messenger-design.md)**。
...
Portal 已经是所有 WS 的管理中心，天然就是消息枢纽。不需要 peer mesh 或独立服务。

### GSD 扩展：portal-messenger

每个 WS 自动安装，通过 WebSocket 连接到 Portal，提供 3 个 tools：

| Tool | 用途 |
|------|------|
| `portal_send` | 向指定 WS 或 Tech Manager 发消息 |
| `portal_wait` | 非阻塞等待（WebSocket 收到消息时自动通知 Agent） |
| `portal_report_progress` | 汇报进度（started → slice done → PR created） |

### 消息类型

| 类型 | 方向 | 触发时机 |
|------|------|----------|
| `PROGRESS` | 开发 WS → Tech Manager | 开始、Slice 完成、PR 创建 |
| `DEPENDENCY_READY` | Tech Manager → 等待方 WS | 上游里程碑合并后 |
| `REVIEW_REQUEST` | 开发 WS → Tech Manager | PR 创建后 |
| `REVIEW_FEEDBACK` | Tech Manager → 开发 WS | PR 有修改意见 |
| `MILESTONE_COMPLETE` | Tech Manager → broadcast | 验收通过 |
| `QUESTION` | 任意 WS → Tech Manager | 需要协调的问题 |
